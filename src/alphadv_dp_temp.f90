...
!	character*10 name(10)
	...
	real (kind=dp)	:: alphaV,betaV,gammaV,deltaV, rhoV, sigmaV, dummyR, pvalue,atau  ! parameters
...
!	call MNparm(7,'atau',pval(7),0.1d0,0.0d0,0.0d0,perr)
	...
	enddo
	!call cdfchi ( 1, p, q, chi2fov(pval(1),pval(2),pval(3),pval(4),pval(5),pval(6),pval(7))&
	!, 1._dp*(79-binmin-5), status, bound )	
	write(*,*) '------------------'
...
!		s = sbin(bin) + dsbin(bin) / 2.0_dp
	...
	SUBROUTINE PltData(pnts, a, b)
		...
		do i=0,pnts-1 
			...
			call set_param(parr(1),parr(2),parr(3),parr(4),parr(5),parr(6))
			dvdata(i) =  f_DVV(s)
			thspcdata(i) = 2._dp*pi**2 * (spcdata(i) + dvdata(i))
			thdata(i) = 2._dp*pi**2/s * (foptdata(i) - ni_dv(s,parr(1),parr(2),parr(3),&
