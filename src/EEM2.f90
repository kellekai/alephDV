PROGRAM main
	...
	call mnexcm(fcn,'MINimize',    (/10000.0_dp,1.0d-8/),2,perr,0)  ! perform MIGRAD minimization
	call mnexcm(fcn, 'HESse', (/ 2._dp /), 0, perr, 0)
	...
	write(*,*) 'CORRECTION TO FM = ', (exp(2.0d0*(-pval(1)-pval(2)*(sbin(binmin)+0.5d0*dsbin(binmin))))*&
	sin(pval(5)+pval(6)*(sbin(binmin)+0.5d0*dsbin(binmin))))/(exp(-pval(1)-pval(2)*(sbin(binmin)+0.5d0*dsbin(binmin)))*&
	sin(pval(3)+pval(4)*(sbin(binmin)+0.5d0*dsbin(binmin))))
...
!		/ (cosh(gammaV * s**rhoV) - cos(alphaV * s**rhoV + betaV)) )
	...
	!
		...
		ni_dv = ((Exp(a + b*s)*(d*Cos(c + d*s) + b*Sin(c + d*s)))/(b**2 + d**2) + (d1*Cos(c1 + d1*s) + &
		2.0d0*b*Sin(c1 + d1*s))/(4.0d0*b**2 + d1**2))/Exp(2.0d0*(a + b*s))
...
!		s = sbin(bin) + dsbin(bin) / 2.0_dp
	...
	SUBROUTINE PltData(pnts, a, b)
		...
		open(55, file='data/PltDataThEEM2.dat')
		...
		close(55)
		open(55, file='data/PltDataExEEM2.dat')
