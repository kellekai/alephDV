...
!	call mnexcm(fcn, 'SCAn', (/ 6._dp, 40.0d0,pval(6) - 0.1d0*pval(6),pval(6) + 0.1d0*pval(6)  /), 4, perr, 0)
	...
	write(*,*) 'CORRECTION TO FM = ',exp(-pval(3)-pval(4)*(sbin(binmin)+0.5*dsbin(binmin)))*pval(5)
...
!write(*,*) truecovv(58,58)
	...
	!
		...
		do n = binmin, 78
			VFO(n) = IntegralPTFO(n, atau) - ni_DVV(n, a0,b0,c0,d0,a1) - FESR(n)
		end do
...
!		/ (cosh(gammaV * s**rhoV) - cos(alphaV * s**rhoV + betaV)) )
		...
		f_DVV =	sin( alpha + beta * s ) * ( exp(-c0 -gamma * s ) + c1 * exp( -2.0d0*gamma * s) )
	...
	!
		...
		ni_dv = ((c1 + 2.0d0*Exp(c + d*s))*Sin(a + b))/(2.0d0*d*Exp(2.0d0*(c + d*s)))
