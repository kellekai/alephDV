PROGRAM main
	...
	call mnexcm(fcn,'FIX', (/1.0_dp,2._dp,3._dp,4._dp,5._dp,6._dp/), 6, perr, 0)
...
!	call mnexcm(fcn, 'SCAn', arg, 0, perr, 0)
	...
	!call mnexcm(fcn, 'HESse', (/ 2._dp /), 0, perr, 0)
	call mnexcm(fcn,'MINimize',    (/10000.0_dp,1.0d-2/),2,perr,0)  ! perform MIGRAD minimization
!	do n=1,7
...
!	call MNparm(6,'delta1',4._dp*pval(4),ierr(4),0.0d0,0.0d0,perr)
	!call mnexcm(fcn, 'SET STRategy', (/ 2._dp /), 1, perr, 0)
	call mnexcm(fcn,'RELeas', (/1.0_dp,2._dp,3._dp,4._dp,5._dp,6._dp/), 6, perr, 0)
	call mnexcm(fcn, 'HESse', (/ 2._dp /), 0, perr, 0)
...
!	call mnexcm(fcn, 'HESse', (/ 2._dp /), 0, perr, 0)
	...
	call get_ERR(7, pval, ierr, fval, 1.0d-4		)

	write(*,*) '=== ERRORS BY INV. THE HESSIAN COMP. WITH FINITE DIFFERENCES EPS = 1.0E-4 ==='
...
!	END FUNCTION alphaS
	...
	SUBROUTINE PltData(pnts, a, b)
		...
		open(55, file='data/PltDataThGEMCIPT.dat')
		...
		close(55)
		open(55, file='data/PltDataExGEMCIPT.dat')
	...
	SUBROUTINE get_ERR(npar, pa, pe, func, eps)
		...
		do n = 1, npar
			...
			fF		= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5),pa(6),pa(7))
			pa(n)	= dum(n) * (1.0d0 - eps)
			fB		= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5),pa(6),pa(7))
			...
			do m = n+1 , npar
				...
				fFfF	= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5),pa(6),pa(7))
				pa(n)	= dum(n) * (1.0d0 - eps)
				pa(m)	= dum(m) * (1.0d0 - eps)
				fBfB	= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5),pa(6),pa(7))
				pa(n)	= dum(n) * (1.0d0 + eps)
				pa(m)	= dum(m) * (1.0d0 - eps)
				fFfB	= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5),pa(6),pa(7))
				pa(n)	= dum(n) * (1.0d0 - eps)
				pa(m)	= dum(m) * (1.0d0 + eps)
				fBfF	= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5),pa(6),pa(7))
